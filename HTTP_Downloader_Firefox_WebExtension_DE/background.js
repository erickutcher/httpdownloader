...
function SendDownloadToClient( download_info )
{
	...
	if ( request )
	{
		...
		request.onerror = function( e )
		{
			...
			CreateDownloadWindow( download_info, "Beim senden der Anfrage für den Download ist ein Fehler aufgetreten." );
		};

		request.ontimeout = function( e )
		{
			//console.log( "The connection has timed out while sending the download request." );
			CreateDownloadWindow( download_info, "Die Zeit für den Aufbau der Verbindung wurde überschritten." );
		};
		...
		request.onload = function( e )
		{
			if ( request.responseText != "DOWNLOADING" )
			{
				...
				CreateDownloadWindow( download_info, "Der Server hat mit einer ungültiger Antwort auf die Anfrage geantwortet." );
			}
		};
	...
	{
		console.log( "XMLHttpRequest konnte nicht erstellt werden." );
	}
}
...
.then( function( options )
{
	...
	browser.contextMenus.create(
	{
		...
		title: "Alle Links herunterladen...",
		contexts: [ "page", "frame", "link", "image", "audio", "video" ]
	} );
